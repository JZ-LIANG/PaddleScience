...
def pre_allocate_program(startup_prog):
    def get_shape(size_in_gb):
        return (size_in_gb // 4, int(7.5 * 1024), 1024, 1024)

    block = startup_prog.global_block()
    shape = get_shape(4)
    variable = block.create_var(
        name="biggggggggggg_tensor",
        shape=shape,
        dtype="float32",
        persistable=False,
        stop_gradient=True)
...
def add_allreduce(prog, params_grads):
    ...
    for i , op in enumerate(ops):
        ...
        if op.output_arg_names[0] in grads_names:
            grads_names.remove(op.output_arg_names[0])
            var_name = op.output_arg_names[0]
            allreduce_op = block._insert_op(
                i + 1,
                type='c_allreduce_sum',
...
def init_algo():
    ...
    # discretize geometry
    # single
    # geo_disc = geo.discretize(npoints=180000, method="sampling")
    # dp2
...
def slove_static():
    ...
    # single
    # for gbsz in [172608, 172608, 172608, 3415, 3415, 3415, 3415,3415, 3415, 3415]:
    # dp2
    for gbsz in [348306, 348306, 348306, 3415, 3415, 3415, 3415,3415, 3415, 3415]:
        lbsz = gbsz // nranks
        # last rank would contain more data
        start_idx = rank * lbsz
        end_idx = (rank + 1) * lbsz
    ...
    with paddle.static.program_guard(main_program, startup_program):
        ...
        if prim_enabled():
            if nranks > 1:
                add_allreduce(main_program, param_grads)
    ...
    pre_allocate_program(startup_program)
    exe.run(startup_program)
    # debug_program(startup_program, "./large_startup_program.txt.")
    print("startup_prog" * 80)
    # time.sleep(60)
    ...
    for i in range(num_time_step):
        ...
        for k in range(train_epoch):
            ...
            print("autograd epoch: " + str(k + 1), "    loss:", out[0], "time: ", time.time() - time1)
        next_uvwp = out[1:]
        # # Save vtk
